defmodule BlennyTestApp.Blenny.FormAuthTest do
  use BlennyTestAppWeb.ConnCase, async: false
  ...
  describe "Crypto.derive_key/2" do
    test "returns a 64-character hex string" do
      hash = BlennyTestApp.Blenny.FormAuth.Crypto.derive_key("password", "salt")
      assert String.match?(hash, ~r/^[0-9a-f]{64}$/)
    end

    test "same inputs produce same hash" do
      a = BlennyTestApp.Blenny.FormAuth.Crypto.derive_key("hello", "world")
      b = BlennyTestApp.Blenny.FormAuth.Crypto.derive_key("hello", "world")
      assert a == b
    end

    test "different passwords produce different hashes" do
      a = BlennyTestApp.Blenny.FormAuth.Crypto.derive_key("password1", "salt")
      b = BlennyTestApp.Blenny.FormAuth.Crypto.derive_key("password2", "salt")
      assert a != b
    end
  end
