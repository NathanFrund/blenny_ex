defmodule BlennyTestApp.Blenny.FormAuth do
  @moduledoc """
  ...
  defp seed_admin(mod, pid) do
    case mod.find_by_username(pid, "admin") do
      nil ->
        hash = BlennyTestApp.Blenny.FormAuth.Crypto.derive_key("admin123", "admin")
  ...
  def handle_sign_in(conn, %{"username" => username, "password" => password}) do
    ...
    case mod.find_by_username(pid, username) do
      ...
      user ->
        hash = BlennyTestApp.Blenny.FormAuth.Crypto.derive_key(password, user.username)
  ...
  def handle_register(conn, %{"username" => username, "password" => password} = params) do
    ...
    cond do
      ...
      true ->
        {mod, pid, _blob} = :persistent_term.get(@store_key)
        hash = BlennyTestApp.Blenny.FormAuth.Crypto.derive_key(password, username)
